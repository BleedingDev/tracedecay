---
name: rn-cases-state
overview: "Author saved-state and owner isolation comparison cases. Preserve the complete original upstream Native implementation from 570 and its operation boundaries."
todos:
...
This lane owns shared-host extension regressions for ordinary/reserved cursor round-trip, exact-CAS rejection and locator conflict/rollback/reopen. The session fixture lane owns sealed-source admission, catch-up and refresh cases. Keep extension-only outcomes separate from 570 Native equivalence and preserve current safety behavior.
...
Prerequisites: rn-reference, rn-contract. The exact edges in ../execution-selection.json are authoritative; this section supersedes older prerequisite prose. Read ../READINESS-PLAN.md for the latest NCM repair and semantic scope.
