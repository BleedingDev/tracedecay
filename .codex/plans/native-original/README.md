...
**Status: planning only.** No product changes or builds ran while preparing this plan. All 28 execution nodes are pending.

## The implementation decision

- Keep the original Native memory and LCM implementation and restore the changed retrieval-anchor authority exactly to `b3b43410e47115056f2066449aafa1822bbb6049`, already merged into the audited product head.
...
[SOURCE-BOUNDARY.md](SOURCE-BOUNDARY.md) records the protected engine, the exact retrieval-anchor restoration and existing shared host extensions. The source owner completes the hunk inventory before interface work. Existing privacy, source-identity and cursor safety fixes remain intact and receive separate regression checks.
...
The [execution graph](execution-graph.mmd) contains **28 nodes and 55 dependency edges**, with **up to 12 independent nodes**. Launch every ready, disjoint lane immediately; the displayed waves are a map, not synchronization barriers.
...
1. Start the independent comparison runner, source inventory/documentation and NCM regression preparation together. The source inventory releases contracts and the exact anchor restoration.
...
| [rn-restore-anchor](execution/rn-restore-anchor.plan.md) | Restore the original retrieval-anchor authority exactly |
...
Use [ORCHESTRATION.md](ORCHESTRATION.md) for exact graph targeting, launch rules, conflict ownership and the Luna handoff template. The saved [handoff bundle](execution-handoff.json) and [selection](execution-selection.json) contain all 55 edges and exact paths.
...
Native equivalence and NCM usefulness are separate results. Existing intermittent NCM recall remains an explicit caveat until evidence establishes otherwise. A successful smoke test does not erase earlier failures.
