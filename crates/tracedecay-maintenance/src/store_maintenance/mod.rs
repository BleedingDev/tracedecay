...
/// Bounded replay journey probes for the daemon's assembled-store fixtures.
#[cfg(feature = "test-helpers")]
pub mod test_helpers {
    pub use super::graph_replay::{
        ReconcileOutcome, defer_graph_replay_pool_busy, reconcile_graph_replay_releases,
        replay_pool_is_held,
    };
}
...
pub async fn run_semantic_vector_generation_retention(
    ...
    let Some(configuration) = lease
        ...
        return match lease.configuration_runtime().client().current().await {
        ...
        tracedecay_code_index_runtime::code_index_scheduler::semantic_vector_graph::ProjectSemanticVectorRetentionStep::Ready(
            ...
            if convergence_pending {
                MaintenanceTickOutcome::Continue(
                    crate::tick::MaintenanceContinuation::SemanticVectorRetention,
                )
            } else {
                MaintenanceTickOutcome::Complete
            }
...
pub fn semantic_retrieval_profiles_disabled(semantic: &SemanticConfig) -> bool {
...
pub enum VectorRetentionInventoryV1 {
    Online {
...
pub async fn resolve_vector_retention_inventory(
    lease: &ProjectStoreMaintenanceLeaseV1,
    schedulers: &CodeIndexSchedulerRegistryV1,
    observations: &StoreTelemetrySamplingRegistry,
    ...
    let vector_provider = schedulers
        .semantic_vector_graph_provider(lease.project_root())
        .await;
    ...
    let expected_vector_revision = match observations
        .semantic_vector_retention_read(lease.project_root())
    {
        crate::telemetry::SemanticVectorRetentionReadV1::Observed { receipt } => receipt.revision,
        crate::telemetry::SemanticVectorRetentionReadV1::SemanticUnseated => {
        ...
        crate::telemetry::SemanticVectorRetentionReadV1::Scanning => {
            return VectorRetentionInventoryV1::CensusScanning;
        }
        crate::telemetry::SemanticVectorRetentionReadV1::Unknown => {
    ...
    let project_root = lease.store_layout().project_root.clone();
...
pub fn classify_vector_readable_sources(
...
pub async fn run_code_generation_retention(
    ...
    let vector_inventory =
        resolve_vector_retention_inventory(lease, schedulers, observations).await;
...
pub async fn apply_code_generation_retention(
    lease: &ProjectStoreMaintenanceLeaseV1,
    schedulers: &CodeIndexSchedulerRegistryV1,
    observations: &StoreTelemetrySamplingRegistry,
    ...
    if let Some(failure) = vector_inventory.degraded_reason() {
        log_code_generation_retention_degraded(observations, lease.project_root(), &failure);
    }
    ...
    let plan = match plan {
        ...
        Ok(Err(
            ...
            log_code_generation_retention_degraded(observations, lease.project_root(), "retention_cancelled");
            return CodeGenerationRetentionOutcomeV1::Failed;
        }
        Ok(Err(
        ...
        Err(_) => {
            log_code_generation_retention_degraded(observations, lease.project_root(), "retention_task_panicked");
            return CodeGenerationRetentionOutcomeV1::Failed;
        }
    };
        ...
        let pinned_vector_sources =
            ...
            .await
            {
                ...
                tracedecay_code_index_runtime::code_index_scheduler::semantic_vector_graph::ProjectVectorReadableSources::ResetRequired(
                    ...
                    log_code_generation_retention_degraded(observations, lease.project_root(), &format!(
...
pub async fn run_code_index_scope_reconciliation(
    ...
    let vector_receipt = match observations.semantic_vector_retention_read(lease.project_root()) {
        crate::telemetry::SemanticVectorRetentionReadV1::Observed { receipt } => receipt,
        crate::telemetry::SemanticVectorRetentionReadV1::Unknown
        | crate::telemetry::SemanticVectorRetentionReadV1::Scanning => {
    ...
    if let Some(replay) = pending_binding_cleanup {
        ...
        let current_inputs =
            match collect_scope_root_proof_inputs(lease, schedulers, &vector_receipt).await {
                Ok(inputs) => inputs,
                Err(failure) => {
                    log_code_index_scope_reconciliation_degraded(failure);
                    return false;
