...
async fn a_retired_semantic_vector_staging_object_is_reset_required_on_every_stamp() {
    for (retired, ddl) in [
        ...
        for stamped in [SCHEMA_VERSION, 35, 34] {
            let (conn, _dir) = create_schema_db().await;
...
async fn seed_payload(conn: &Connection, ordinal: usize, content: &str) {
    let fact_id = format!("fact.v2.{ordinal}");
    let assertion_id = format!("assertion.v2.{ordinal}");
    conn.execute(
...
async fn deleting_a_payload_drops_its_digest_and_digests_never_update() {
    ...
    conn.execute_batch("DELETE FROM memory_v2_assertion_payloads WHERE fact_id = 'fact.v2.1';")
        .await
    ...
    assert_eq!(
        remaining,
        vec!["fact.v2.0".to_owned(), "fact.v2.2".to_owned()]
    );
}
