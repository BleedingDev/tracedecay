-- Exact v35 project-store DDL snapshot used for migration admission.
-- This checked-in snapshot is immutable history; changes to current schema
-- installers must not redefine the released admission shape.
...
CREATE INDEX idx_read_cache_session
...
CREATE INDEX idx_retrieval_anchor_dispositions_current
...
CREATE INDEX idx_retrieval_anchor_reverse_derivative
...
CREATE TRIGGER retrieval_anchors_immutable_update
    BEFORE UPDATE ON retrieval_anchors BEGIN
        SELECT RAISE(ABORT, 'retrieval anchors are immutable');
    END;
CREATE TRIGGER retrieval_anchors_immutable_delete
...
CREATE TRIGGER retrieval_anchor_derivative_tombstones_immutable_update
    ...
    END;
CREATE TRIGGER retrieval_anchor_derivative_tombstones_immutable_delete
...
CREATE TRIGGER memory_v2_payloads_no_update
        ...
        END;
CREATE TABLE memory_v2_assertion_payload_purges (
            assertion_id TEXT NOT NULL,
            fact_id TEXT NOT NULL,
            owner_kind TEXT NOT NULL,
            project_id TEXT NOT NULL,
...
CREATE TRIGGER memory_v2_assertion_payload_purges_no_update
...
CREATE TABLE memory_v2_automatic_fact_receipts (
...
CREATE INDEX idx_memory_v2_automatic_fact_receipt_list
...
CREATE TRIGGER memory_v2_automatic_fact_receipts_no_update
        ...
        END;
CREATE TRIGGER memory_v2_automatic_fact_receipts_no_delete
        BEFORE DELETE ON memory_v2_automatic_fact_receipts BEGIN
            SELECT RAISE(ABORT, 'memory_v2 automatic fact receipts are immutable');
        END;
CREATE TABLE memory_v2_assertion_payload_digests (
...
CREATE INDEX memory_v2_assertion_payload_digests_lookup
            ON memory_v2_assertion_payload_digests(
                owner_kind, project_id, content_digest, fact_id
            );
CREATE TRIGGER memory_v2_assertion_payload_digests_no_update
        ...
        END;
CREATE TRIGGER memory_v2_payloads_digest_delete
        AFTER DELETE ON memory_v2_assertion_payloads BEGIN
...
CREATE INDEX idx_memory_v2_operation_receipts_fact
            ON memory_v2_operation_receipts(
                fact_id, owner_kind, project_id, recorded_at
            );
CREATE INDEX idx_memory_v2_operation_receipts_automation_run
            ON memory_v2_operation_receipts(
                owner_kind, project_id, operation_kind,
                json_extract(receipt_json, '$.automation_run_id'),
                recorded_at, operation_id
            );
CREATE INDEX idx_memory_v2_automatic_fact_receipts_automation_run
            ON memory_v2_automatic_fact_receipts(
...
CREATE TRIGGER memory_v2_operation_receipts_no_delete
        BEFORE DELETE ON memory_v2_operation_receipts BEGIN
            SELECT RAISE(ABORT, 'memory_v2 operation receipts are immutable');
        END;
CREATE TRIGGER memory_v2_operation_receipts_no_payload
...
CREATE TRIGGER memory_v2_feedback_history_no_delete
        ...
        END;
CREATE TRIGGER memory_v2_automatic_fact_receipts_require_keys
     ...
     END;
CREATE INDEX idx_memory_v2_current_search
...
CREATE INDEX idx_evidence_occurrences_anchor
        ON evidence_source_occurrences(owner_digest, source_anchor_id);
CREATE INDEX idx_evidence_occurrences_timeline
...
CREATE INDEX idx_external_source_states_owner_v1
    ON external_source_states_v1(owner_kind, owner_id, source_id);
CREATE TABLE external_source_definition_revisions_v1 (
    source_id TEXT NOT NULL,
    definition_revision INTEGER NOT NULL CHECK (definition_revision > 0),
    definition_digest TEXT NOT NULL,
...
CREATE TABLE external_source_binding_revisions_v1 (
...
CREATE TABLE external_source_authority_receipts_v1 (
    binding_id TEXT NOT NULL,
    idempotency_key TEXT NOT NULL,
    request_digest TEXT NOT NULL,
    definition_digest TEXT NOT NULL,
...
CREATE TABLE external_source_frontiers_v1 (
    binding_id TEXT NOT NULL,
    frontier_digest TEXT NOT NULL,
    frontier_json TEXT NOT NULL,
    PRIMARY KEY (binding_id, frontier_digest)
);
CREATE TABLE external_source_commit_receipts_v2 (
    binding_id TEXT NOT NULL,
    idempotency_key TEXT NOT NULL,
    request_digest TEXT NOT NULL,
    definition_revision INTEGER NOT NULL CHECK (definition_revision > 0),
...
CREATE TABLE external_source_mutations_v1 (
    binding_id TEXT NOT NULL,
...
CREATE TABLE external_source_lineage_v1 (
    binding_id TEXT NOT NULL,
...
CREATE TABLE external_source_objects_v2 (
    binding_id TEXT NOT NULL,
    native_object_digest TEXT NOT NULL,
...
CREATE TABLE external_source_pending_projections_v1 (
    binding_id TEXT NOT NULL,
    predecessor_frontier_digest TEXT NOT NULL,
    successor_frontier_digest TEXT NOT NULL,
...
CREATE TABLE external_source_projection_publications_v2 (
    binding_id TEXT NOT NULL,
...
CREATE TABLE external_source_projection_effects_v2 (
    binding_id TEXT NOT NULL,
    projection_digest TEXT NOT NULL,
...
CREATE TABLE external_source_projection_lineage_v1 (
    binding_id TEXT NOT NULL,
    projection_digest TEXT NOT NULL,
...
CREATE TABLE external_source_projected_objects_v2 (
    binding_id TEXT NOT NULL,
    native_object_digest TEXT NOT NULL,
    mutation_digest TEXT NOT NULL,
    PRIMARY KEY (binding_id, native_object_digest)
);
CREATE TABLE external_source_acquisition_queue_v1 (
    binding_id TEXT PRIMARY KEY,
    state_digest TEXT NOT NULL,
    not_before_micros INTEGER,
    state_json TEXT NOT NULL
);
CREATE INDEX idx_external_source_acquisition_ready_v1
...
CREATE INDEX idx_generation_diagnostics_generation_state
        ON generation_diagnostics (generation_id, record_state);
CREATE INDEX idx_generation_diagnostics_anchor_revision
        ON generation_diagnostics (diagnostic_anchor, publication_revision DESC);
CREATE INDEX idx_generation_diagnostics_generation_state_anchor
        ON generation_diagnostics (generation_id, record_state, diagnostic_anchor);
CREATE INDEX idx_generation_diagnostics_file
        ON generation_diagnostics (file_occurrence_id, generation_id);
CREATE INDEX idx_generation_diagnostics_file_generation_state_anchor
...
CREATE INDEX idx_diagnostic_publications_generation_revision
        ON diagnostic_generation_publications (generation_id, publication_revision DESC);
CREATE UNIQUE INDEX idx_diagnostic_generation_current
...
CREATE INDEX idx_graph_publication_replay_projection_sequence
    ON graph_publication_replay_v1(shard_id, namespace, projection, sequence);
CREATE TABLE graph_publication_replay_dependencies_v1 (
...
CREATE INDEX idx_graph_publication_dependency_replay
    ON graph_publication_replay_dependencies_v1(dependency_replay_sequence);
CREATE TABLE graph_publication_replay_tombstones_v1 (
...
CREATE INDEX idx_graph_publication_tombstone_projection
    ON graph_publication_replay_tombstones_v1(shard_id, namespace, projection);
CREATE TABLE graph_publication_replay_tombstone_dependencies_v1 (
...
CREATE TABLE graph_verified_heads_v1 (
    shard_id TEXT NOT NULL,
    namespace TEXT NOT NULL,
    projection TEXT NOT NULL,
...
CREATE TABLE semantic_vector_stages (
...
CREATE INDEX idx_semantic_vector_live_source_generation
    ON semantic_vector_stages(shard_id, source_generation)
    WHERE state IN ('pending', 'ready_to_publish', 'published');
CREATE INDEX idx_semantic_vector_live_source_scope
    ON semantic_vector_stages(shard_id, source_scope)
      WHERE state IN ('pending', 'ready_to_publish', 'published');
CREATE INDEX idx_semantic_vector_code_scope_binding
    ON semantic_vector_stages(shard_id, code_scope_hash, source_scope)
    WHERE state IN ('pending', 'ready_to_publish', 'published');
CREATE INDEX idx_semantic_vector_published_project_generation
    ON semantic_vector_stages(shard_id, semantic_generation_id)
    WHERE state = 'published';
CREATE INDEX idx_semantic_vector_project_census
    ON semantic_vector_stages(shard_id, stage_id);
CREATE INDEX idx_semantic_vector_projection_census
    ON semantic_vector_stages(shard_id, namespace, projection, stage_id);
CREATE TABLE semantic_vector_stage_census_authority (
    shard_id TEXT PRIMARY KEY,
    revision INTEGER NOT NULL CHECK (revision > 0)
) STRICT;
CREATE TABLE semantic_vector_stage_adoption_authority (
    shard_id TEXT PRIMARY KEY,
    revision INTEGER NOT NULL CHECK (revision > 0)
) STRICT;
CREATE TABLE semantic_vector_source_scope_bindings (
    shard_id TEXT NOT NULL,
    code_scope_hash TEXT NOT NULL
        CHECK (length(code_scope_hash) = 64
            AND code_scope_hash NOT GLOB '*[^0-9a-f]*'),
...
CREATE TRIGGER semantic_vector_stage_census_after_scope_binding_insert
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_scope_binding_delete
...
BEGIN
    ...
    ON CONFLICT(shard_id) DO UPDATE SET revision=revision+1;
END;
CREATE TRIGGER semantic_vector_source_scope_binding_immutable
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_stage_insert
...
BEGIN
    ...
    ON CONFLICT(shard_id) DO UPDATE SET revision=revision+1;
END;
CREATE TRIGGER semantic_vector_stage_census_after_stage_update
...
BEGIN
    ...
    ON CONFLICT(shard_id) DO UPDATE SET revision=revision+1;
END;
CREATE TRIGGER semantic_vector_stage_adoption_after_state_update
...
BEGIN
    ...
    ON CONFLICT(shard_id) DO UPDATE SET revision=revision+1;
END;
CREATE TRIGGER semantic_vector_stage_census_after_stage_delete
...
BEGIN
    ...
    ON CONFLICT(shard_id) DO UPDATE SET revision=revision+1;
END;
CREATE TABLE semantic_vector_retirement_cleanup (
...
CREATE INDEX idx_semantic_vector_pending_retirement_cleanup
    ON semantic_vector_retirement_cleanup(shard_id, cleanup_id);
CREATE TRIGGER semantic_vector_stage_publication_identity_guard
...
END;
CREATE TABLE semantic_vector_stage_batches (
...
CREATE TRIGGER semantic_vector_stage_census_after_batch_insert
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_batch_update
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_batch_delete
...
END;
CREATE TABLE semantic_vector_stage_chunk_receipts (
    stage_id INTEGER NOT NULL
        REFERENCES semantic_vector_stages(stage_id) ON DELETE RESTRICT,
...
CREATE TRIGGER semantic_vector_stage_census_after_chunk_insert
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_chunk_update
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_chunk_delete
...
END;
CREATE TABLE semantic_vector_stage_graph_effects (
...
CREATE TRIGGER semantic_vector_stage_census_after_effect_insert
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_effect_update
...
END;
CREATE TRIGGER semantic_vector_stage_census_after_effect_delete
...
END;
CREATE INDEX idx_semantic_vector_pending_effects
    ON semantic_vector_stage_graph_effects(state, outbox_sequence);
CREATE TABLE handoff_open_grants_v1 (
...
CREATE TABLE td_runtime_writer_idempotency_v2 (
    shard_json TEXT NOT NULL,
    incarnation INTEGER NOT NULL CHECK (incarnation > 0),
    authority_epoch INTEGER NOT NULL CHECK (authority_epoch > 0),
    idempotency_key TEXT NOT NULL,
...
CREATE TABLE td_runtime_writer_outbox_v1 (
...
CREATE INDEX td_runtime_writer_outbox_ordering_v1
...
CREATE UNIQUE INDEX td_runtime_writer_outbox_effect_v1
ON td_runtime_writer_outbox_v1 (source_shard_json, effect_id);
CREATE INDEX td_runtime_writer_outbox_state_v1
ON td_runtime_writer_outbox_v1 (
    source_shard_json,
    source_incarnation,
    source_authority_epoch,
...
CREATE TABLE td_runtime_writer_inbox_v1 (
...
CREATE INDEX td_runtime_writer_inbox_ordering_v1
...
CREATE UNIQUE INDEX td_runtime_writer_inbox_effect_v1
