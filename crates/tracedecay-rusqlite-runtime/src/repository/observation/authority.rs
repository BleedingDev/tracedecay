...
pub(super) fn verify_observation_authority(
    ...
    let stored = connection
        ...
        .optional()?;
    let Some((availability, capture, anchor_id, owner, origin_json)) = stored else {
    ...
    let stored = (availability, capture, anchor_id, owner);
    let expected = (
        encode(attachment.availability())?,
        attachment.provenance().map(encode).transpose()?,
        attachment
    ...
    if stored != expected {
        let (availability, capture, anchor_id, owner) = stored;
