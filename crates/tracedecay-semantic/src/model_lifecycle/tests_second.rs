...
fn isolated_evaluation_import_never_follows_a_catalog_member_symlink() {
    ...
    let result = copy_local_evaluation_member(&package_root, "model.onnx", destination.path());
...
fn restart_re_admits_explicit_import_without_legacy_acquisition() {
    ...
    restarted
        .mark_ready(&restarted.lifecycle_mutation_target().unwrap())
        .unwrap();
...
fn explicit_import_atomically_retains_and_restores_the_verified_rollback_artifact() {
    ...
    assert_eq!(
        ...
        "rollback must invalidate the prior artifact projection target"
    );
    let active = owner
        .artifact_store
...
#[test]
fn rollback_from_failed_current_rotates_previous_pointer_and_lease() {
    let fixture = tempfile::tempdir().unwrap();
    let (catalog, model_id) = tiny_catalog(fixture.path());
    let model = catalog.get(&model_id).unwrap().clone();
    let root = tempfile::tempdir().unwrap();
    ...
    let first = owner
        .import_local_artifact(&model_id, &first_manifest, fixture.path(), 10)
        .unwrap();
    let first_digest = first.state.as_ref().unwrap().artifact_digest().to_owned();
    owner
        .mark_ready(&owner.lifecycle_mutation_target().unwrap())
        .unwrap();
    let mut second_manifest = first_manifest;
    ...
    let second = owner
        .import_local_artifact(&model_id, &second_manifest, fixture.path(), 20)
        .unwrap();
    let second_digest = second.state.as_ref().unwrap().artifact_digest().to_owned();
    owner
        .mark_ready(&owner.lifecycle_mutation_target().unwrap())
        .unwrap();
    owner
        .mark_runtime_failed(
            ...
            "runtime failed after publication",
            true,
        )
        .unwrap();

    let mut events = owner.verified_ready_events();
    let prior_event_epoch = events.borrow().epoch;
    let rolled_back = owner.rollback_to_previous().unwrap();
    assert_eq!(
        rolled_back.state.as_ref().unwrap().artifact_digest(),
        first_digest
    ...
    assert!(events.borrow_and_update().epoch > prior_event_epoch);
    assert!(matches!(
        owner
            .inner
            .read()
        ...
        Some(SemanticModelLifecycleStateV1::Installed {
            artifact_digest,
            ..
        }) if artifact_digest == &second_digest
    ));
    assert_eq!(
        owner
            .artifact_store
...
#[test]
fn shared_failed_current_reconstructs_lease_and_rollback_after_restart() {
    ...
    owner
        .import_local_artifact(&model_id, &tiny_manifest(&model), &fixture, 10)
        .unwrap();
    owner
        .mark_ready(&owner.lifecycle_mutation_target().unwrap())
        .unwrap();
    let first = owner.status().state.unwrap();
    let first_inventory_digest = owner
        .artifact_store
        .installed_digest(install_path_of(&first).unwrap())
        .unwrap();

    let mut second_manifest = tiny_manifest(&model);
    second_manifest.payload.resource_ceiling.max_resident_bytes += 1;
    owner
        .import_local_artifact(&model_id, &second_manifest, &fixture, 20)
        .unwrap();
    owner
        .mark_ready(&owner.lifecycle_mutation_target().unwrap())
        .unwrap();
    let second = owner.status().state.unwrap();
    let second_inventory_digest = owner
        .artifact_store
        .installed_digest(install_path_of(&second).unwrap())
        .unwrap();
    owner
        .mark_runtime_failed(
            &owner.lifecycle_mutation_target().unwrap(),
    ...
    assert_eq!(
        owner
            .artifact_store
            .artifact_digest_for_lease(&active_lease, ArtifactLeaseKindV1::Active, 30,)
            .unwrap(),
        Some(second_inventory_digest.clone())
    );

    // Drop the lease authority to model a crash after the Failed state was
    // persisted but before the shared inventory pointer was reconciled.
    owner
        .artifact_store
    ...
    assert_eq!(
        reopened
            ...
            .unwrap(),
        Some(second_inventory_digest.clone()),
        "restart must reconstruct the failed shared artifact lease"
    );
    assert!(reopened.status().remediation.rollback);

    let rolled_back = reopened.rollback_to_previous().unwrap();
    let rolled_back_path = install_path_of(rolled_back.state.as_ref().unwrap()).unwrap();
    assert_eq!(
        reopened.artifact_store.installed_digest(rolled_back_path),
        Some(first_inventory_digest.clone())
    );
    assert_eq!(
        reopened
            .artifact_store
...
fn pinned_reranker_cold_first_and_warm_queries_reuse_one_session() {
    ...
    let package_bytes = manifest
        ...
        .sum::<u64>();
    manifest.payload.resource_ceiling.max_model_bytes = manifest.payload.model_member.byte_length;
    ...
    let candidates = ["session pooling", "unrelated rendering"]
        ...
        .collect::<Vec<_>>();
    let snapshot =
        tracedecay_domain::CandidateSetDigest::new(format!("sha256:{}", "a".repeat(64))).unwrap();
...
impl ExplicitHttpsArtifactTransportV1 for FixtureRerankerHttpsTransport {
    ...
    ) -> Result<super::super::artifact_store::HttpsArtifactRangeResponseV1, ArtifactImportErrorV1>
    {
        let bytes = self
            .members
            .iter()
            .find_map(|(path, bytes)| request.url.ends_with(&format!("/{path}")).then_some(bytes))
...
fn settings_change_schedules_acquire_to_installed_without_blocking_semantics_flag() {
    ...
    owner
        ...
        .unwrap();
    owner
        .mark_indexing(&owner.lifecycle_mutation_target().unwrap(), 1, 2)
        .unwrap();
    owner
        .mark_ready(&owner.lifecycle_mutation_target().unwrap())
        .unwrap();
...
fn runtime_failure_retains_ready_rollback_across_restart() {
    ...
    let owner =
        SemanticModelLifecycleOwnerV1::open(root.path(), catalog.clone(), source.clone()).unwrap();
    owner.select_model(Some(&model_id), true).unwrap();
    owner.acquire_blocking_for_tests().unwrap();
    owner
        .mark_ready(&owner.lifecycle_mutation_target().unwrap())
...
fn hermetic_download_failure_reports_failed_not_warming() {
    ...
    let root = tempfile::tempdir().unwrap();
    let owner =
        SemanticModelLifecycleOwnerV1::open(root.path(), catalog, Arc::new(RefusedNetworkSource))
            .unwrap();
    owner.select_model(Some(&model_id), true).unwrap();
    assert!(owner.enqueue_demand_acquisition_if_needed());
    assert_eq!(
...
fn verified_background_install_reopens_offline_without_acquisition() {
    ...
    let online =
        SemanticModelLifecycleOwnerV1::open(root.path(), catalog.clone(), online_source).unwrap();
    ...
    let reopened =
        SemanticModelLifecycleOwnerV1::open(root.path(), catalog, offline_source.clone()).unwrap();
...
#[test]
fn scoped_selection_recomputes_shared_inventory_after_joined_worker_publish() {
    let temp = tempfile::tempdir().unwrap();
    let fixture = temp.path().join("fixture");
    let (catalog, model_id) = tiny_catalog(&fixture);
    // Keep the source channels in the test so the worker can be released
    // only after selection has completed its preflight scan.
    let (entered_tx, entered_rx) = mpsc::sync_channel(1);
    let (release_tx, release_rx) = mpsc::sync_channel(1);
    let source = Arc::new(BlockingFixtureSource {
        root: temp.path().join("fixture"),
        calls: AtomicUsize::new(0),
        entered: entered_tx,
        release: Mutex::new(release_rx),
    });
    let owner = Arc::new(
        SemanticModelLifecycleOwnerV1::open_scoped(
            temp.path().join("owner"),
            temp.path().join("artifacts"),
            "profile/project-race",
            catalog,
            source.clone(),
        )
        .unwrap(),
    ...
    entered_rx
        .recv_timeout(Duration::from_secs(1))
        .expect("shared acquisition entered fixture source");

    let preflight_entered = Arc::new(Barrier::new(2));
    let preflight_release = Arc::new(Barrier::new(2));
    owner.set_mutation_pause_after_preflight_scan_for_tests(
        Arc::clone(&preflight_entered),
        Arc::clone(&preflight_release),
    );
    let selecting_owner = Arc::clone(&owner);
    let selecting_model_id = model_id.clone();
    let (selected_tx, selected_rx) = mpsc::sync_channel(1);
    let selecting = thread::spawn(move || {
        selected_tx
            .send(selecting_owner.select_model(Some(&selecting_model_id), false))
            .expect("selection result receiver");
    });
    preflight_entered.wait();

    release_tx.send(()).expect("release shared acquisition");
    let deadline = std::time::Instant::now() + Duration::from_secs(1);
    loop {
        let published_shared = owner.status().state.as_ref().is_some_and(|state| {
            matches!(state, SemanticModelLifecycleStateV1::Installed { install_path, .. }
                        if install_path.starts_with(temp.path().join("artifacts")))
        });
        let finished = owner
            .worker
            .lock()
        ...
        if published_shared && finished {
            break;
        }
        assert!(
            std::time::Instant::now() < deadline,
            "shared worker did not publish before post-join selection"
        );
        thread::yield_now();
    }
    ...
    let selected = selected_rx
        ...
        .expect("selection after shared publication");
    selecting.join().expect("selection caller");
    assert!(matches!(
        selected.state,
        Some(SemanticModelLifecycleStateV1::Installed { ref install_path, .. })
            if install_path.starts_with(temp.path().join("artifacts"))
    ));
    assert_eq!(
        source.calls.load(Ordering::SeqCst),
        5,
        "the joined worker must be reused rather than reacquired"
    );
}
...
fn scoped_model2vec_acquisition_preserves_catalog_member_paths_for_reuse() {
    ...
    assert_eq!(
        reused.state.as_ref().and_then(install_path_of),
        Some(install)
    );
    assert_eq!(source.calls.load(Ordering::SeqCst), fetched);
    #[cfg(feature = "semantic-model2vec")]
    {
        ...
        assert_eq!(
            environment.runtime,
            crate::model2vec_adapter::MODEL2VEC_RUNTIME_FAMILY_V1
        );
    }
}

#[test]
fn lifecycle_artifact_identity_survives_persistence_and_legacy_projection_reopen() {
    ...
    let persisted = authority
        .projection()
        .clone()
        .with_lifecycle_artifact_identity(identity.clone());
    let canonical_digest = persisted
        .embedding_key()
        .canonical_digest()
        .expect("canonical projection digest");
    assert_eq!(
        persisted.projection_key().profile_digest,
        canonical_digest,
        "the admitted projection must expose the embedded canonical digest"
    );

    let durable = tempfile::NamedTempFile::new().expect("projection persistence fixture");
    fs::write(
        durable.path(),
        serde_json::to_vec_pretty(&persisted).expect("persist projection"),
    )
    .expect("write projection persistence fixture");
    let reopened: tracedecay_domain::AdmittedEmbeddingProjectionKeyV1 =
        serde_json::from_slice(&fs::read(durable.path()).expect("read projection persistence"))
            .expect("reopen persisted projection");
    assert_eq!(
        reopened.lifecycle_artifact_identity(),
        Some(identity.as_str()),
        "the owner-issued lifecycle identity must survive durable reopen"
    );

    let mut legacy_value = serde_json::to_value(&persisted).expect("encode projection value");
    assert!(
        legacy_value
            .as_object_mut()
            .expect("projection object")
            .remove("lifecycle_artifact_identity")
            .is_some(),
        "the modern fixture must contain the optional lifecycle identity"
    );
    fs::write(
        durable.path(),
        serde_json::to_vec_pretty(&legacy_value).expect("persist legacy projection"),
    )
    .expect("write legacy projection fixture");
    let legacy: tracedecay_domain::AdmittedEmbeddingProjectionKeyV1 =
        serde_json::from_slice(&fs::read(durable.path()).expect("read legacy projection"))
            .expect("decode legacy projection without lifecycle identity");
    assert_eq!(
        legacy.lifecycle_artifact_identity(),
        None,
        "legacy projections must remain unbound when the optional field is absent"
    );
    assert_eq!(
        legacy.projection_key(),
        persisted.projection_key(),
        "legacy decoding must preserve the admitted projection key"
    );
    assert_eq!(
        legacy
            .embedding_key()
            .canonical_digest()
            .expect("legacy canonical projection digest"),
        canonical_digest,
        "adding lifecycle metadata must not re-key the legacy projection"
    );
}
