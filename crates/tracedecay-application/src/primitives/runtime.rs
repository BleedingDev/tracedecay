...
// (`tracedecay_contracts::retrieval`) so the catalog contribution can
...
async fn dispatch_admitted(
    ...
    match invocation.request {
        ...
        PrimitiveRequest::SessionLookup(request) => {
            ...
            {
                Ok(outcome) => outcome,
                Err(tracedecay_contracts::retrieval::TemporalRetrievalFailure::Unavailable) => {
                    return problem(
                        &context,
                        &operation,
                ...
                Err(tracedecay_contracts::retrieval::TemporalRetrievalFailure::ResetRequired) => {
                    return problem(
                        &context,
                        &operation,
                ...
                Err(
                    tracedecay_contracts::retrieval::TemporalRetrievalFailure::StructuralRefusal(
                        refusal,
                    ),
                ) => {
                    return problem(
...
fn grep_page<T: Serialize>(
    ...
    operation: &ApplicationOperation,
    domain: EvidenceDomain,
    page: tracedecay_contracts::retrieval::grep_analysis::PrimitivePageV1<T>,
    partial: bool,
) -> Result<ApplicationResult<Value>, ApplicationContractError> {
