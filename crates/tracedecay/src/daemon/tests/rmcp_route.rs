...
async fn unix_action_prepare_then_tools_call_emits_one_daemon_receipt() {
    let fixture = rmcp_route_fixture("unix-action-receipt-wire").await;
    let profile_root = fixture._temp.path().join("profile");
    // The production daemon publishes this record before accepting any
    // authenticated control exchange. Keep the lease alive for both socket
    // connections so the MCP server can bind the same generation on prepare
    // and terminal dispatch.
    let _authority = tracedecay_daemon_identity::authority::DaemonAuthority::acquire(
        &profile_root,
        &tracedecay_daemon_protocol::DaemonEndpoint::Unix(fixture._temp.path().join("daemon.sock")),
        "action-receipt-wire-test",
    )
    .expect("publish test daemon authority");
    let scope = tracedecay_daemon_identity::authority::canonical_identity_path(
        fixture
            .handshake
            .project_path
            .as_ref()
    ...
    .expect("canonical fixture project scope")
    .to_string_lossy()
    .into_owned();
    let tool = "tracedecay_status";
    let route = "status";
    let entrypoint = tracedecay_daemon_protocol::action_receipt::ACTION_ENTRYPOINT_MCP_STDIO;
    let arguments = json!({});
    let action_digest = tracedecay_daemon_protocol::action_receipt::canonical_action_digest(
        &scope, tool, entrypoint, &arguments,
    );
    ...
    let proof_key = "11".repeat(32);
    let expires_at = now + 60_000_000;
    let prepare = json!({
        "jsonrpc": "2.0",
        "id": "action-prepare-wire",
        "method": "initialize",
        "params": {
            "protocolVersion": "2025-06-18",
            "capabilities": {},
            "clientInfo": {"name": "action-receipt-wire-test", "version": "1"},
            "_meta": {
                "nativeOriginalActionPrepare": {
                    "format": tracedecay_daemon_protocol::action_receipt::ACTION_RECEIPT_FORMAT,
                    "revision": tracedecay_daemon_protocol::action_receipt::ACTION_RECEIPT_REVISION,
                    "action_id": "action-prepare-wire",
                    "route": route,
                    "nonce": nonce,
                    "proof_key": proof_key,
    ...
    let (server_stream, client_stream) =
        tokio::net::UnixStream::pair().expect("prepare socket pair");
    ...
    writer
        .write_all(b"\n")
        .await
        .expect("prepare handshake newline");
    write_line(&mut writer, &prepare).await;
    let prepare_response = read_value(&mut reader, "action prepare response timed out").await;
    assert_eq!(
        prepare_response["id"],
        json!("action-prepare-wire"),
        "unexpected action-prepare response: {prepare_response}"
    );
    let acknowledgement = &prepare_response["result"]["_meta"]
        [tracedecay_daemon_protocol::action_receipt::ACTION_PREPARE_META_KEY];
    assert_eq!(
        acknowledgement["action_id"],
        json!("action-prepare-wire"),
        "unexpected action-prepare acknowledgement: {prepare_response}"
    );
    assert_eq!(acknowledgement["route"], json!(route));
    assert_eq!(acknowledgement["nonce"], json!(nonce));
    assert_eq!(acknowledgement["action_digest"], json!(action_digest));
    assert_eq!(acknowledgement["scope"], json!(scope));
    assert_eq!(acknowledgement["expires_at"], json!(expires_at));
    assert!(acknowledgement.get("proof_key").is_none());
    // The one-shot prepare endpoint closes its side after the acknowledgement;
    // dropping the write half also handles an already-disconnected peer.
    drop(writer);
    prepare_task
        .await
        .expect("join action prepare server")
        .expect("serve action prepare socket");

    let (server_stream, client_stream) =
        tokio::net::UnixStream::pair().expect("tools call socket pair");
    let engine = fixture.engine.clone();
    ...
    writer
        .write_all(
            fixture
                .handshake
                .to_line()
                .expect("call handshake")
                .as_bytes(),
        )
        .await
        .expect("write call handshake");
    writer
        .write_all(b"\n")
        .await
        .expect("call handshake newline");
    write_line(&mut writer, &initialize_request()).await;
    let initialize_response = read_value(&mut reader, "call initialize response timed out").await;
        ...
        &json!({
            ...
            "params": {
                ...
                "_meta": {
                    ...
                    "nativeOriginalActionRoute": route,
    ...
    .await;
    drop(writer);
    let call_response = read_value(&mut reader, "tools call response timed out").await;
    assert_eq!(call_response["id"], json!(2));
    let receipt = call_response["result"]["_meta"]
        [tracedecay_daemon_protocol::action_receipt::ACTION_RECEIPT_META_KEY]
        .clone();
    assert!(
        receipt.is_object(),
        "action-backed terminal response must carry an object receipt: {call_response}"
    );
    assert_eq!(
        receipt["action_id"],
        json!("action-prepare-wire"),
        "unexpected action terminal response: {call_response}"
    );
    assert_eq!(receipt["route"], json!(route));
    assert_eq!(receipt["nonce"], json!(nonce));
    assert_eq!(receipt["action_digest"], json!(action_digest));
    assert_eq!(receipt["scope"], json!(scope));
    assert_eq!(receipt["tool"], json!(tool));
    assert_eq!(receipt["entrypoint"], json!(entrypoint));
    assert_eq!(receipt["expires_at"], json!(expires_at));
    assert!(
        receipt["receipt_mac"]
            .as_str()
    ...
    call_task
        .await
        .expect("join action call server")
        .expect("serve action call socket");
}
