...
async fn checked_out_declared_stack_conflict_enqueues_without_moving_refs() {
    if run_in_fixture_home(
        "native_integration::journey_tests::checked_out_declared_stack_conflict_enqueues_without_moving_refs",
    ) {
        return;
    }
...
async fn generated_only_change_requires_semantic_review() {
    if run_in_fixture_home(
        "native_integration::journey_tests::generated_only_change_requires_semantic_review",
    ) {
        return;
    }
    let directory = tempfile::tempdir().expect("temporary project directory");
