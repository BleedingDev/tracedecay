...
admitted session.message_committed.v1 observation
  ...
  → advisory recall candidate attested under checkout_observations
```

The staged store is provider-local state that TraceDecay's own Native provider owns. It writes no canonical fact table, and the acknowledgement follows the commit rather than preceding it, so an acknowledged observation always has a row behind it. Retention evicts payload content but keeps a tombstone, so an evicted key still answers duplicate instead of re-staging. Checkout recall reads the newest bounded window (at most the per-scope retention ceiling, 512 by default) matching all five checkout columns and `tombstone = 0`, then verifies each row against its own seven-field digest before scoring. This changes only Native candidate admission: storage/idempotency, original receipts and source identity, the requesting session scope, and host hydration authority remain exact. Origin session metadata cannot become a `session:` citation for another session; staged content remains provider-attested `Available`.
