...
Allowed files are the nine exact paths named in the machine-readable policy: the six daemon shutdown call-chain files, plus the call chain's own unit tests under `crates/tracedecay/src/daemon/invocation_tests/`. They may propagate one caller-supplied absolute deadline, bound blocking drain by its remaining budget, report an incomplete shutdown as `Failed` while still reaching terminal teardown, and assert that ordering through an observable effect of `begin_shutdown` under a bounded wait.
...
This category's caps are 8 files and 360 changed lines, raised from 6 and 320 by [ADR-0015](../architecture/adr/ADR-0015-daemon-shutdown-test-fence-and-supersession-headroom.md) against a measured 7 files and 329 changed lines. ADR-0015 supersedes [ADR-0013](../architecture/adr/ADR-0013-daemon-shutdown-touch-point-expansion.md) as the approving decision named in the category's `cap_revision` block; ADR-0013 remains the historical record of the 5/287 to 6/320 step, which approved the sixth path and the exact numbers before the change that needs them. Bounding the LSP lease join from the caller in `invocation/lsp.rs` was not enough: `LspLeaseTaskRegistry::shutdown` in `crates/tracedecay-daemon-service/src/invocation/types.rs` removes every lease task from the registry before it joins them one at a time, so a caller-side `timeout_at` dropped that future with the remaining tasks neither cancelled nor aborted, and a dropped `JoinHandle` detaches rather than aborts. The deadline therefore has to reach the registry itself, which is one more file in the same shutdown call chain.
